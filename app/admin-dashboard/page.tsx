...
interface Lead {
  ...
  consent: boolean
  source: string
  pageUrl?: string      // NEW: Full URL
  pageTitle?: string    // NEW: Page title
  referrer?: string     // NEW: Referrer
...
export default function LeadsTable({
  ...
  const filteredLeads = sortedLeads.filter((lead) => {
    ...
    const matchesTreatment =
      treatmentFilter === "all" || lead.treatment === treatmentFilter
    const matchesDate =
  ...
  const getFormBadge = (formName: string) => {
    ...
    const config = formConfig[normalized] || formConfig.default
    return (
...
const exportToCSV = () => {
  const headers = [
    ...
    "Status",
    "Form Name",
    "Source URL",        // Changed from "Source"
    "Page Title",        // NEW
    "Referrer",          // NEW
    "TeleCRM Synced",
    "Created At",
  ...
  const csvData = filteredLeads.map((lead) => [
    ...
    lead.status || "",
    lead.formName || "",
    lead.pageUrl || lead.source || "",  // Prefer pageUrl over source
    lead.pageTitle || "",
    lead.referrer || "",
  ...
  const getFormLabel = (formName: string) => {
    const normalized = formName.trim().toLowerCase()

    if (normalized === "Website leads") return "Hair Treatments"
    if (normalized === "Website leads") return "Cosmetic Surgery"
    if (normalized === "contact-form") return "Contact Form"
        ...
        <CardContent className="p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
            {Object.entries(formStats).map(([formName, stats]) => {
              const label = getFormLabel(formName)
          ...
          <div className="grid grid-cols-1 md:grid-cols-6 gap-4 mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
            ...
            <Select value={treatmentFilter} onValueChange={setTreatmentFilter}>
              ...
              <SelectContent className="bg-white border-gray-300 text-gray-900">
                ...
                {/* You can keep static options OR build from leads dynamically */}
                <SelectItem value="Baldness" className="focus:bg-gray-100">
                  Baldness
                </SelectItem>
                <SelectItem
                  value="Hair Thinning"
                ...
                <SelectItem value="Liposuction" className="focus:bg-gray-100">
                  Liposuction
                </SelectItem>
                <SelectItem value="Rhinoplasty" className="focus:bg-gray-100">
                  Rhinoplasty
                </SelectItem>
                <SelectItem value="Gynecomastia" className="focus:bg-gray-100">
                  Gynecomastia
                </SelectItem>
                <SelectItem
                ...
                <SelectItem value="Other" className="focus:bg-gray-100">
                  Other
                </SelectItem>
              </SelectContent>
            </Select>
          ...
          <div className="space-y-2 text-gray-700">
            ...
            {lead.userAgent && (
              <div>
                ...
                <span className="text-xs">{lead.userAgent.substring(0, 100)}...</span>
              </div>
            )}
            {lead.telecrmId && (
              <div>
        ...
        <div>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mt-4 text-sm text-gray-600 gap-2">
            <div>
              ...
              {formFilter !== "all" && ` • Form: ${getFormLabel(formFilter)}`}
            </div>
